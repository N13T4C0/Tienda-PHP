<div class="formulario ancho">
    ...
    <form method="POST" action="<?= URL_BASE ?>/admin/guardarProducto" enctype="multipart/form-data">
        ...
        <div class="campo">
            <label for="imagen">Imagen del producto <small>(JPG, PNG, GIF o WEBP)</small></label>
            <?php if ($producto && !empty($producto->imagen) && $producto->imagen !== 'sin-imagen.svg'): ?>
                <p>Imagen actual: <strong><?= htmlspecialchars($producto->imagen) ?></strong> &mdash; sube un archivo nuevo solo si quieres cambiarla</p>
            <?php endif; ?>
